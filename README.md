...
I made up a programming language and named it "NRJ". NRJ is like a (much much) worse version of Java.
...
`nrj.asm` is long and bloated. There are lots of consecutive `push` and `pop` operations, registers are not used well, etc, etc. That's because all the code above is programmatically generated and this compiler does no optimization whatsoever. 
...
As you can see, there's a big gap between the `gcd.asm` file automatically generated by the compiler and the handwritten `handwritten_gcd.asm` file. That's one of many differences between a toy compiler and a real one!
...
This compiler really has no practical purpose whatsoever. It satisfies the definition of a compiler in that it translates a program written in a high level language into a low level language. Beyond that, it is lacking many of the features present in a real compiler. There is no error recovery, no support for classes and other OOP structures, no support for any data types other than integers, and no assembly optimization. 

At some point I do want to return to this project and add support for arrays, since I am interested in how compilers deal with data structures stored on the heap.
